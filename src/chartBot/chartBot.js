...
const ChartApp = () => {
	...
	const renderTime = (props) => (
		<Time
		  ...
		  textStyle={{
			right: {
			  color: 'red', // Color for user's message time
			},
			left: {
			  color: 'white', // Color for bot's message time
			},
		  }}
		/>
	  );
	const renderBubble = (props) => {
		return (
			<Bubble
				...
				textStyle={{
					left: {
						color: 'white', // text color for bot's messages
					},
					right: {
						color: 'black', // text color for user's messages
					},
				}}
				wrapperStyle={{
					left: {
						backgroundColor: '#1154de', // background color for bot's messages
					},
					right: {
						backgroundColor: '#ebecef', // background color for user's messages
					},
				}}
			/>
		);
